const FORM_PATH_READINESS = [
  ...
  { level: 3, label: 'Completed draft may be prepared after review', userMeaning: 'A completed draft may be prepared only after official-source verification, missing-answer checks, and Human Review Specialist approval.' },
...
function evaluateFormPathReadiness(path, facts){
  ...
  if (path.readinessLevel >= 3 && completionPercent === 100) reviewerStatus = 'draft organized for human review';
  return {
    pathId: path.id,
