...
11. The exact inherited nested release carrier occasionally reports a transient file count while the workspace filesystem is still converging after extraction. PRE128 recognizes the retained predecessor count-error wordings, retries the full hash-pinned reconstruction up to six times with increasing short waits, and never changes or bypasses a required count or archive hash.
...
15. Production verification treats configured capability and live capability as different facts. Optional AI is publicly available only after a successful current-process provider check; a failed or absent check leaves the rules-based experience open and the optional AI lane truthfully unavailable. The public check response contains no provider, model, credential, feature-control, or internal error detail.
16. Public-copy transformations must preserve grammar and whole-word meaning. PRE128 uses an adjective-safe replacement for inherited `source-linked` copy and complete-phrase boundaries for source/review wording; it rejects the mechanical phrases and substring corruption found during live visual QA. These are copy corrections only—the established homepage layout and first encounter remain unchanged.
...
The first successful PRE128 promotion, deployment `dep-dacpo3f40ujc73er399g` at Git commit `f00814c7992ca9d6e036925fb8e2766c18a9cb9f`, exposed a narrower production-verification truth gap: configuration-based status said optional AI was available while the fixed synthetic live check returned unavailable. No user matter was used, rules-based help remained open, and no provider setting was changed. PRE128 now requires successful live verification before advertising optional AI availability; see `release-evidence/PRE128_PRODUCTION_AI_AVAILABILITY_CORRECTION.md`.

Desktop visual QA of deployment `dep-dacpubfqj5pc738nv070` at Git commit `2e2be35f2d816bac0cd7a267ba0673c7748101eb` found a public-copy transformation defect without a layout regression. A broad adjective replacement produced ungrammatical phrases, and an unbounded `review boundary` replacement corrupted the distinct phrase `Preview boundary`. PRE128 now uses grammar-compatible and complete-phrase transformations with full-public-tree regression scans; see `release-evidence/PRE128_PUBLIC_COPY_TRANSFORMATION_CORRECTION.md`.
