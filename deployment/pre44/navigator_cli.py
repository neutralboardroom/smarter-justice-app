...
def _storage_config():
    explicit = str(os.environ.get('SJ_NAVIGATOR_DB') or os.environ.get('SJ_NAVIGATOR_DB_PATH') or '').strip()
    data_dir = str(os.environ.get('SJ_NAVIGATOR_DATA_DIR') or os.environ.get('DATA_DIR') or '').strip()
    if explicit:
        path = pathlib.Path(explicit).expanduser()
        source = 'EXPLICIT_DB_PATH'
    elif data_dir:
        path = pathlib.Path(data_dir).expanduser() / 'smarter-justice' / 'navigator-pre44.sqlite3'
        source = 'CONFIGURED_DATA_DIR'
    else:
        path = pathlib.Path('/tmp/sj-navigator-pre44.sqlite3')
        source = 'RUNTIME_LOCAL_TMP'
    backup_dir = str(os.environ.get('SJ_NAVIGATOR_BACKUP_DIR') or '').strip()
    durable_requested = str(os.environ.get('SJ_NAVIGATOR_DURABLE_STORAGE') or '').strip().lower() in ('1','true','yes','on')
    temp_like = str(path).startswith('/tmp/') or str(path).startswith('/var/tmp/')
    durable_accepted = bool(durable_requested and not temp_like)
    return {
        'path': path,
        'source': source,
        'backupDir': pathlib.Path(backup_dir).expanduser() if backup_dir else None,
        'durableRequested': durable_requested,
        'durableAccepted': durable_accepted,
        'mode': 'DURABLE_SQLITE' if durable_accepted else 'RUNTIME_LOCAL_SQLITE',
    }
...
def fake_transport(endpoint, headers, body, timeout):
    return 200, {}, {
        ...
        'output': [{'type': 'message', 'content': [{'type': 'output_text', 'text': 'PRE44 NAVIGATOR TEST READY'}]}],
        'usage': {'input_tokens': 12, 'output_tokens': 7},
    }


class Pre44OpenAIProvider(OpenAIProvider):
    def responses_create(self, *, tool_id, input_text, request_id, safety_identifier, timeout=30):
        ...
        body = {
            ...
            'metadata': {'sj_tool_id': tool_id, 'sj_request_id': request_id, 'sj_release': 'pre44'},
        }
        try:
...
def provider():
    ...
    if test_mode:
        ...
        return Pre44OpenAIProvider(transport=fake_transport, env=env)
    return Pre44OpenAIProvider()
...
def maybe_backup():
    ...
    if not backup_dir or not cfg['path'].exists():
        return {'configured': bool(backup_dir), 'written': False}
    backup_dir.mkdir(parents=True, exist_ok=True)
    target = backup_dir / 'navigator-pre44-latest.sqlite3'
    tmp = backup_dir / 'navigator-pre44-latest.sqlite3.tmp'
    shutil.copy2(cfg['path'], tmp)
...
def build_model_input(history, prompt, mode, actor_class, page_context=''):
    lines = [BASE_SYSTEM, MODE_INSTRUCTIONS[mode], f'User class: {actor_class}.']
    if page_context:
        lines.append('Smarter Justice page context: ' + page_context[:500])
...
def chat(payload):
    ...
    try:
        ...
        if requested_thread:
            ...
            thread_id = store.create_thread(owner_scope=actor_class, owner_ref=actor, tenant_id=None, experience_mode='SMARTER_JUSTICE')
        ...
        if str(decision.get('state', '')).startswith('BLOCK_'):
            return {'ok': False, 'errorCode': 'AI_ALLOWANCE_BLOCKED', 'message': 'This Navigator allowance has reached its current safety limit. Sign in or try again after the allowance resets.', 'threadId': thread_id, 'allowance': decision}

        store.add_message(thread_id, owner_ref=actor, tenant_id=None, role='USER', input_mode=input_mode, content=prompt)
        request_id = 'pre44-' + uuid.uuid4().hex
        result = provider().responses_create(
            tool_id='NAVIGATOR_' + mode,
            input_text=build_model_input(history, prompt, mode, actor_class, page_context),
        ...
        receipt = ledger.record(
            request_id=request_id, tenant_id=None, actor_type=actor_class, actor_ref=actor,
            tool_id='NAVIGATOR_' + mode, provider=str(result.get('provider') or 'OPENAI'),
            model=str(result.get('model') or os.environ.get('OPENAI_API_MODEL') or ''),
            usage_class='CORE_INCLUDED_AI', input_units=int(usage.get('inputUnits') or 0), output_units=int(usage.get('outputUnits') or 0),
        )
        backup = maybe_backup()
        cfg = _storage_config()
        return {
            'ok': True, 'threadId': thread_id, 'answer': answer, 'toolMode': mode, 'actorClass': actor_class,
            'provider': 'openai', 'model': result.get('model'), 'providerResponseId': result.get('providerResponseId'),
            'latencyMs': int(result.get('latencyMs') or 0), 'usage': usage,
            'usageLedgerRecorded': bool(receipt.get('recorded') or receipt.get('idempotentReplay')),
            'allowanceState': decision.get('state'), 'inputMode': input_mode, 'storeRequested': False,
            'externalActionsExecuted': False, 'publicAiEnabled': True, 'automaticOverageBillingEnabled': False,
            'persistenceMode': cfg['mode'], 'durablePersistenceAccepted': cfg['durableAccepted'], 'backupWritten': backup['written'],
        }
    finally:
        conn.close()
...
def status(payload):
    ...
    try:
        AIUsageLedger(conn)
        NavigatorStore(conn)
        rows = conn.execute('SELECT thread_id,created_at FROM navigator_threads WHERE owner_ref=? ORDER BY created_at DESC LIMIT 10', (actor,)).fetchall()
        readiness = provider().readiness()
        readiness['ready'] = bool(readiness.get('keyPresent') and readiness.get('modelPresent') and readiness.get('projectIdPresent'))
        cfg = _storage_config()
        return {
            'ok': True, 'providerReadiness': readiness, 'actorClass': actor_class,
            'availableToolModes': list(MODE_INSTRUCTIONS.keys()),
            'threads': [{'threadId': r['thread_id'], 'createdAt': r['created_at']} for r in rows],
            'publicAiEnabled': True, 'externalActionsExecuted': False, 'automaticOverageBillingEnabled': False,
            'requestStorageRequested': False, 'persistenceMode': cfg['mode'],
            'persistenceSource': cfg['source'], 'durablePersistenceRequested': cfg['durableRequested'],
            'durablePersistenceAccepted': cfg['durableAccepted'], 'backupConfigured': bool(cfg['backupDir']),
            'persistenceBoundary': 'Durable only when a non-temporary Navigator path is explicitly configured and SJ_NAVIGATOR_DURABLE_STORAGE=true. Otherwise the service truthfully reports runtime-local SQLite.',
        }
    finally:
        conn.close()
...
def main():
    try:
        ...
        if action == 'chat': out = chat(payload)
        elif action == 'status': out = status(payload)
        else: raise ValueError('unknown Navigator action')
