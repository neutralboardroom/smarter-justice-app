...
TEST_DATA=ROOT/'.pre44-persistence-test-data'
...
try:
    env=dict(os.environ)
    env.update({'NODE_ENV':'test','SJ_NAVIGATOR_TEST_MODE':'1','OPENAI_API_KEY':'qualification-not-secret','OPENAI_API_MODEL':'gpt-5-mini','OPENAI_PROJECT_ID':'proj_qualification','SJ_NAVIGATOR_DATA_DIR':str(TEST_DATA),'SJ_NAVIGATOR_DURABLE_STORAGE':'true'})
    ...
    db=TEST_DATA/'smarter-justice'/'navigator-pre44.sqlite3'
    assert db.exists()
finally:
    shutil.rmtree(TEST_DATA,ignore_errors=True)
