...
function inferPractice(question, selected){
  ...
  const rules = [
    ...
    ['domestic-violence-protection-orders',['domestic violence','intimate partner violence','abusive partner','abuse by my partner','controlling partner','protection order','restraining order','order of protection','family offense','stalking me','following me','threatened me','violated the order']],
...
function buildStartingPoint({question, practiceArea, subcategory, state, county, city, attachments=[], documentType='', smartAnswers={}}){
  ...
  const formReadiness = {
    ...
    userMeaning: pathReadiness?.userMeaning || (bestReadiness <= 1 ? 'This path should start as an organized worksheet or starting summary. Completed forms should not be shown until the official instructions and required fields are confirmed.' : 'This path may support a future draft, but the required review must occur before a draft is shown.')
  };
  const nextSteps = [];
  ...
  return {
    ...
    plainLanguageStartingPoint: `This looks like a ${practice.name.toLowerCase()} starting point. Smarter Justice can help organize what happened, save documents when that service is available, identify likely next steps, and prepare a clear summary for review.`,
    concerns,
    nextSteps,
