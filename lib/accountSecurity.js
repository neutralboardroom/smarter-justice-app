...
function cookie(name, token, options = {}) {
  ...
  return `${name}=${clear ? '' : encodeURIComponent(token)}; Path=${options.path || '/'}; ${httpOnly ? 'HttpOnly; ' : ''}SameSite=${sameSite}; Max-Age=${maxAge}${secure ? '; Secure' : ''}`;
}

const CSRF_COOKIE = 'sj_csrf';
function generateCsrfToken() { return crypto.randomBytes(32).toString('base64url'); }
function csrfCookie(token, clear = false) {
  return cookie(CSRF_COOKIE, token, { clear, maxAgeSeconds:8 * 60 * 60, sameSite:'Strict', httpOnly:false });
}
function csrfTokenFromRequest(req) { return parseCookies(req)[CSRF_COOKIE] || ''; }
...
function validateCsrfRequest(req, options = {}) {
  const cookieToken = csrfTokenFromRequest(req);
  const headerToken = String(req?.headers?.['x-csrf-token'] || '').trim();
  if (!timingSafeTextEqual(cookieToken, headerToken)) return { ok:false, error:'Your secure session check could not be confirmed. Refresh the page and try again.' };
  const fetchSite = String(req?.headers?.['sec-fetch-site'] || '').toLowerCase();
  if (fetchSite === 'cross-site') return { ok:false, error:'Cross-site requests are not allowed.' };
  const origin = String(req?.headers?.origin || '').trim();
  if (origin) {
    try {
      const allowed = new Set((options.allowedOrigins || []).filter(Boolean).map(value => new URL(value).origin));
      if (options.requestHost) {
        const protocol = options.requestProtocol || 'https:';
        allowed.add(`${protocol}//${options.requestHost}`);
      }
      if (!allowed.has(new URL(origin).origin)) return { ok:false, error:'This request did not come from an approved Smarter Justice page.' };
    } catch { return { ok:false, error:'This request did not include a valid page origin.' }; }
  }
  return { ok:true };
}
...
module.exports = {
  clean, normalizeEmail, hashToken, hashPassword, verifyPassword, parseCookies, cookie, CSRF_COOKIE, generateCsrfToken, csrfCookie, csrfTokenFromRequest, timingSafeTextEqual, validateCsrfRequest,
