...
function status(){
  const state=readState(); cleanupSessions(state);
  const bootstrapEmailPresent=Boolean(security.normalizeEmail(process.env.OWNER_ACCOUNT_EMAIL || ''));
  const bootstrapPasswordPresent=Boolean(String(process.env.OWNER_ACCOUNT_PASSWORD || ''));
  const legacyTokenAllowed=process.env.NODE_ENV !== 'production' || /^(true|1|yes)$/i.test(String(process.env.ALLOW_LEGACY_OWNER_TOKEN || ''));
  const mfaEnabledAccounts=state.accounts.filter(item=>item.mfa?.enabled).length;
  return {
    accountCount:state.accounts.length,
    activeSessions:state.sessions.length,
    mfaEnabledAccounts,
    accountsWithRecoveryCodes:state.accounts.filter(item=>(item.mfa?.recoveryCodeHashes || []).length>0).length,
    accountAuthenticationReady:state.accounts.length>0,
    mfaRequiredForAll:state.accounts.length>0 && mfaEnabledAccounts===state.accounts.length,
    recoveryReadyForAll:state.accounts.length>0 && state.accounts.every(item=>!item.mfa?.enabled || (item.mfa?.recoveryCodeHashes || []).length>0),
    bootstrapCredentialsPresent:bootstrapEmailPresent || bootstrapPasswordPresent,
    bootstrapCredentialsRemoved:state.accounts.length>0 && !bootstrapEmailPresent && !bootstrapPasswordPresent,
    legacyTokenAllowed,
