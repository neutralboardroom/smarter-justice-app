...
const marker = path.join(target, '.pre122-render-bootstrap.json');
if (!fs.existsSync(marker)) {
  console.error('[PRE122 DEPLOY] marker missing');
  process.exit(1);
}
const metadata = JSON.parse(fs.readFileSync(marker, 'utf8'));
if (metadata.release !== 'v2.0.0-pre122' || metadata.baseRelease !== 'v2.0.0-pre121' || metadata.productAuthority !== 'SMARTER_JUSTICE_ONLY' || metadata.navigatorOrCommunityMutation !== false || metadata.productionDeploymentAuthorized !== true) {
  console.error('[PRE122 DEPLOY] marker mismatch');
  process.exit(1);
}
const env = {
  ...process.env,
  ...
  PYTHONPYCACHEPREFIX: process.env.PYTHONPYCACHEPREFIX || '/tmp/sj-pre122-runtime-pycache',
};
