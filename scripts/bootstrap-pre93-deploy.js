...
function fail(message){console.error(`[PRE93-D2 DEPLOY] ${message}`);process.exit(1)}
...
const d1=readJson('deployment/pre93/PRE93_D1_DEPLOYMENT_AMENDMENT_RECEIPT.json');assert(d1.release==='v2.0.0-pre93','D1 release mismatch');assert(d1.nationalRogerRulePreserved===true,'national Roger Rule D1 assertion missing');assert(d1.search?.everyFieldOptional===true,'optional-field search D1 assertion missing');assert(d1.search?.paidOrganicBoost===false,'D1 organic ranking boundary failed');
for(const row of d1.changedFiles||[]){const file=path.join(target,row.path);assert(fs.existsSync(file),`D1 file missing: ${row.path}`);assert(fs.statSync(file).size===row.bytes,`D1 file size mismatch: ${row.path}`);assert(sha(file)===row.sha256,`D1 file hash mismatch: ${row.path}`)}
const hotfixScript=path.join(root,'scripts','apply-pre93-d2-memory-hotfix.js');assert(fs.existsSync(hotfixScript),'PRE93-D2 hotfix script is missing');cp.execFileSync(process.execPath,[hotfixScript],{stdio:'inherit'});
const d2=readJson('deployment/pre93/PRE93_D2_RUNTIME_HOTFIX_RECEIPT.json');assert(d2.release==='v2.0.0-pre93','D2 release mismatch');assert(d2.deploymentAmendment==='PRE93-D2-MEMORY-SAFE-NATIONAL-ATTORNEY-DIRECTORY','D2 amendment identity mismatch');assert(d2.sourceCarrierSha256===digest,'D2 carrier binding mismatch');assert(d2.factoryVersion==='0.27.0'&&d2.factoryCounts?.total===12578,'D2 Factory binding mismatch');assert(d2.search?.everyFieldOptional===true&&d2.search?.tokenAwarePartialSearch===true&&d2.search?.paidOrganicBoost===false,'D2 search boundary mismatch');assert(d2.nationalRogerRule===seal.NATIONAL_OWNER_RULE,'D2 national Roger Rule mismatch');
for(const row of d2.changedFiles||[]){const file=path.join(target,row.path);assert(fs.existsSync(file),`D2 file missing: ${row.path}`);assert(fs.statSync(file).size===row.bytes,`D2 file size mismatch: ${row.path}`);assert(sha(file)===row.afterSha256,`D2 file hash mismatch: ${row.path}`)}
...
const marker={schemaVersion:'smarter-justice.pre93-d2.render-bootstrap.v1',release:'v2.0.0-pre93',amendment:d2.deploymentAmendment,sourceAmendment:d1.amendment,carrier:path.basename(archive),carrierSha256:digest,carrierBytes:stat.size,factoryVersion:factory.producer.version,factoryCounts:factory.counts,nationalOwnerRule:seal.NATIONAL_OWNER_RULE,preSealRuleLock:seal.PRE_SEAL_RULE_LOCK,memorySafeDirectory:true,preparedAt:new Date().toISOString()};
fs.writeFileSync(path.join(target,'.pre93-d1-render-bootstrap.json'),JSON.stringify(marker,null,2)+'\n');
console.log(`[PRE93-D2 DEPLOY] verified ${marker.release} ${marker.amendment}; Factory=${marker.factoryCounts.total}; carrier=${digest}`);
