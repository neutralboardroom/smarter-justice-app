...
const markerPath = path.join(target, '.pre126-render-bootstrap.json');
if (!fs.existsSync(markerPath)) {
  console.error('[PRE126 DEPLOY] marker missing');
  process.exit(1);
}
const marker = JSON.parse(fs.readFileSync(markerPath, 'utf8'));
if (marker.release !== 'v2.0.0-pre126'
  || marker.baseRelease !== 'v2.0.0-pre125'
  || marker.rollbackCommit !== '2e4b90c083c469bc0e055747258fc9521eed06b2'
  || marker.productAuthority !== 'SMARTER_JUSTICE_ONLY'
  || marker.legalCommunityNetworkMutation !== true
  || marker.homepageAndVisualDirectionAuthorized !== true
  ...
  console.error('[PRE126 DEPLOY] marker mismatch');
  process.exit(1);
}
const env = {
  ...process.env,
  ...
  PYTHONPYCACHEPREFIX: process.env.PYTHONPYCACHEPREFIX || '/tmp/sj-pre126-runtime-pycache'
};
