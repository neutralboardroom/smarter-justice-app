...
const generatedAt=process.env.PRE97_BUILD_TIMESTAMP||'2026-08-18T23:00:00.000Z';
...
function patch(rel,from,to,{required=true}={}){let s=read(rel);const matched=typeof from==='string'?s.includes(from):from.test(s);if(!matched){if(required)fail(`patch anchor missing in ${rel}: ${String(from).slice(0,120)}`);return false}s=s.replace(from,to);write(rel,s);return true}
...
const ownerRuleSrc=path.join(root,'governance/current/ROGER_RULE_PROACTIVE_PAGE_BY_PAGE_AUDIT.md');assert(fs.existsSync(ownerRuleSrc),'PRE97 Roger Rule source missing');fs.mkdirSync(path.join(target,'governance','current'),{recursive:true});fs.copyFileSync(ownerRuleSrc,path.join(target,'governance','current','ROGER_RULE_PROACTIVE_PAGE_BY_PAGE_AUDIT.md'));
...
patch('public/professionals.html','City or borough','City',{required:false});
patch('public/professionals.html','placeholder="Kings County"','placeholder="Example: Cook County"',{required:false});
patch('public/professionals.html','Search the currently configured New York official attorney-registration connector.','Use an available official jurisdiction connector. New York is currently connected here; additional qualified connectors can be added nationally.',{required:false});
...
const directory=read('public/professionals.html');assert(!/City or borough/.test(directory),'regional City or borough wording remains in exact carrier after overlay');assert(directory.includes('/pre97-directory.js'),'national directory overlay missing');
...
write('.pre97-overlay.json',JSON.stringify({release,predecessorSha256:expectedSha,generatedAt,ownerRule:'ROGER_RULE_PROACTIVE_PAGE_BY_PAGE_AUDIT.md',changedFiles:changed.map(x=>x.path)},null,2)+'\n');
fs.mkdirSync(outDir,{recursive:true});const out=path.join(outDir,'PRE97_DEPLOY_RUNTIME.tgz');fs.rmSync(out,{force:true});cp.execFileSync('bash',['-lc',`tar --sort=name --mtime='@0' --owner=0 --group=0 --numeric-owner -cf - -C "${target}" . | gzip -n > "${out}"`],{stdio:'inherit'});const artifact={file:path.basename(out),bytes:fs.statSync(out).size,sha256:shaFile(out),release,predecessorSha256:expectedSha,changedFiles:changed.length,linkAudit:summary.linkAudit};fs.writeFileSync(path.join(outDir,'PRE97_ARTIFACT_RECEIPT.json'),JSON.stringify(artifact,null,2)+'\n');console.log(`[PRE97 BUILD] complete ${artifact.file} sha256=${artifact.sha256} bytes=${artifact.bytes} changed=${changed.length} links=${linkAudit.checked} missing=${linkAudit.missing.length}`);
