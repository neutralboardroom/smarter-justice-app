...
for(const name of files){
 ...
 for(const [domain,dest] of Object.entries(routes)){
   const re=new RegExp(`href=["']https?://(?:www\\.)?${esc(domain)}(?:/[^"']*)?["']`,'ig');
   h=h.replace(re,()=>{converted++;return `href="${dest}"`});
 }
...
const banned=new RegExp(`href=["']https?://(?:www\\.)?(?:${Object.keys(routes).map(esc).join('|')})(?:/[^"']*)?["']`,'i');
const remaining=[];
for(const name of files){const h=fs.readFileSync(path.join(pub,name),'utf8');if(banned.test(h))remaining.push(name);}
