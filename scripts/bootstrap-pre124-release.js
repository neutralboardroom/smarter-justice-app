...
if(marker.release!=='v2.0.0-pre124'||marker.baseRelease!=='v2.0.0-pre123'||marker.productAuthority!=='SMARTER_JUSTICE_ONLY'||marker.navigatorOrCommunityMutation!==false||marker.stripeMutation!==false||marker.paymentSetupDeferredByOwner!==true||marker.productionDeploymentAuthorized!==true)errors.push('marker-boundary');
if(receipt.release!=='v2.0.0-pre124'||receipt.baseRelease!=='v2.0.0-pre123'||receipt.predecessorUnchangedFilesHashVerified!==true||receipt.stripeMutation!==false||receipt.paymentSetupDeferredByOwner!==true||Number(receipt.changes?.publicPagesAudited||0)<100)errors.push('receipt-boundary');
...
const server=fs.existsSync(path.join(root,'server.js'))?fs.readFileSync(path.join(root,'server.js'),'utf8'):'';if(!server.includes('v2.0.0-pre124'))errors.push('server-release');
