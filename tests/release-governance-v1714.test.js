...
assert.equal(pkg.version,'1.7.14');
assert(pkg.scripts.test.includes('professional-directory-search-v1714.test.js'));
assert(pkg.scripts.test.includes('release-governance-v1714.test.js'));
assert(text('server.js').includes("const VERSION = '1.7.14'"));

const manifest=readJson('portal-manifest.json');
assert.equal(manifest.currentDevelopmentVersion,'1.7.14');
assert.equal(manifest.latestZipName,'smarter-justice-v1.7.14.zip');
assert.equal(manifest.lastExactArtifactTestVersion,'1.7.14');
assert.equal(manifest.testSuiteParts,28);
...
assert.equal(improvements.releaseVersion,'1.7.14');
assert.equal(improvements.items.length,42);
...
const evidence=readJson('RELEASE_EVIDENCE_V1.7.14.json');
assert.equal(evidence.release.version,'1.7.14');
assert.equal(evidence.release.rollbackArtifact,'smarter-justice-v1.7.13.zip');
assert.equal(evidence.release.deployment.deployed,false);
assert.equal(evidence.release.deployment.liveVerified,false);
for(const id of ['V1714-NEUTRAL-DIRECTORY-SEARCH','V1714-PRACTICE-TAXONOMY','V1714-SOURCE-FRESHNESS-AND-COUNTS','V1714-FIRM-SOURCE-FACT-PRESERVATION','V1714-ACCESSIBLE-SEARCH-RECOVERY'])assert(evidence.changes.some(x=>x.id===id),`${id} missing`);
...
const readiness=readJson('READINESS_DIMENSIONS_V1.7.14.json');
assert.equal(readiness.releaseVersion,'1.7.14');
assert.equal(readiness.dimensions.length,15);
assert(/27 public professional\/firm records, 23 strict qualifying profiles/.test(readiness.dimensions.find(x=>x.id==='product').evidence));
...
assert.equal(registry.CAPABILITY_REGISTRY_VERSION,'1.0.3');
for(const id of ['neutral-multi-field-professional-search','exact-postal-professional-search','professional-practice-search-taxonomy','qualifying-profile-metrics'])assert(registry.CAPABILITY_DEFINITIONS.some(x=>x.id===id),`${id} missing from capability registry`);
...
const port=portForTest(3974);
const base=`http://127.0.0.1:${port}`;
const tempStorage=fs.mkdtempSync(path.join(os.tmpdir(),'smarter-justice-v1714-'));
const child=spawn(process.execPath,['server.js'],{cwd:root,env:{...process.env,NODE_ENV:'test',PORT:String(port),APP_BASE_URL:base,SMARTER_JUSTICE_STORAGE_DIR:tempStorage,OWNER_CONTROL_CENTER_TOKEN:'owner-v1714-regression-token-1234567890',ADMIN_TOKEN:'admin-v1714-regression-token-1234567890',PORTAL_RULES_API_TOKEN:'rules-v1714-regression-token-1234567890',OWNER_NOTIFICATION_EMAIL:''}});
...
(async()=>{
 try{
  ...
  assert.equal(health.data.version,'1.7.14');
  assert.equal(health.data.sensitiveTrafficApproved,false);
  const directory=await result(`${base}/professionals.html`);assert.equal(directory.response.status,200);assert(/ZIP code/.test(directory.data)&&/More search filters/.test(directory.data));
  ...
  console.log('v1.7.14 neutral professional search, qualifying-profile governance, version records, runtime filters, and closed-gate tests passed.');
