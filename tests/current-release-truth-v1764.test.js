...
(async()=>{
 ...
 assert.equal(pkg.version,'1.7.67');assert.equal(pkg.scripts.test.split(' && ').length,120);
 assert.equal(manifest.currentDevelopmentVersion,'1.7.67');assert.equal(manifest.latestZipName,'smarter-justice-v1.7.65.zip');assert.equal(manifest.dependencyIndependentTestParts,120);assert.equal(manifest.testSuiteParts,121);
 ...
 const v=truth.validate();assert.equal(v.ok,true,v.errors.join('\n'));assert.equal(v.releaseVersion,'1.7.67');assert.equal(v.sourceVersion,'1.7.64');assert.equal(v.launchState,'NO_GO');assert.equal(v.deploymentAuthorized,false);
 const a=authority.validate();assert.equal(a.ok,true,a.errors.join('\n'));assert.equal(a.advancedCount,3);assert.equal(a.unchangedCount,1);assert.equal(a.completeIdentityCount,3);assert.equal(a.versionOnlyCount,1);
 ...
 const cont=fs.readFileSync(path.join(root,'CONTINUATION_PROMPT_V1.7.66.md'),'utf8');assert.equal((cont.match(/^SMARTER JUSTICE$/gm)||[]).length,1);assert(cont.includes('EXPECTED NEXT VERSION IF BASE REMAINS CURRENT: v1.7.66'));assert(cont.includes('smarter-justice-v1.7.65.zip'));assert(cont.includes('MPCR-2026-07-31-E1-R0-05F4296ED378'));assert(cont.trimEnd().endsWith('END OF EIGHTH-PASS REFINED SMARTER JUSTICE REUSABLE VERSION-NEUTRAL COMPLETE IMMEDIATE BUILD MASTER COMMAND'));
 ...
 try{
  ...
  const allowed=await request(base,'/api/owner/current-release-truth',h);assert.equal(allowed.status,200);assert.equal(allowed.data.appVersion,'1.7.67');assert.equal(allowed.data.currentReleaseTruth.validation.ok,true);assert.equal(allowed.data.currentReleaseTruth.selectedBase.version,'1.7.64');
  const cc=await request(base,'/api/owner/control-center',h);assert.equal(cc.status,200);assert.equal(cc.data.version,'1.7.67');assert.equal(cc.data.currentReleaseTruth.validation.ok,true);assert.equal(cc.data.currentReleaseTruth.publicDisplay.currentVersion,'1.7.67');
