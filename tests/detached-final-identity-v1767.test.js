...
process.env.SMARTER_JUSTICE_STORAGE_DIR=fs.mkdtempSync(path.join(os.tmpdir(),'sj-detached-v1767-'));
process.env.OWNER_CONTROL_CENTER_TOKEN='owner-detached-v1767-token-123456789';
...
(async()=>{
 ...
 assert.equal(pkg.version,'1.7.83');assert.equal(pkg.scripts.test.split(' && ').length,145);assert.equal(manifest.dependencyIndependentTestParts,145);assert.equal(manifest.testSuiteParts,146);assert.equal(manifest.capabilities.detachedFinalArtifactIdentityVerification,true);
 const dir=fs.mkdtempSync(path.join(os.tmpdir(),'sj-final-id-v1767-'));const artifact=path.join(dir,'smarter-justice-v1.7.83.zip');const bytes=Buffer.from('detached immutable final identity acceptance fixture');fs.writeFileSync(artifact,bytes);const h=crypto.createHash('sha256').update(bytes).digest('hex');
 const receipt={schema:'smarter-justice-detached-final-artifact-identity',schemaVersion:'1.0.0',receiptId:'SJFIR-V1772-TEST-0001',receiptEpoch:1,receiptRevision:0,identityMode:'DETACHED_AFTER_IMMUTABLE_PACKAGING',issuedAt:'2026-08-01T00:35:00-04:00',sourcePackageVersion:'1.7.83',artifact:{version:'1.7.83',filename:'smarter-justice-v1.7.83.zip',hashAlgorithm:'SHA-256',sha256:h,sizeBytes:bytes.length},launchState:'NO_GO',deploymentAuthorized:false,scope:'FINAL_ARCHIVE_IDENTITY_ONLY_NOT_DEPLOYMENT_OR_LIVE_ACCEPTANCE'};
