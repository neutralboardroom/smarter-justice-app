...
process.env.SMARTER_JUSTICE_STORAGE_DIR=fs.mkdtempSync(path.join(os.tmpdir(),'sj-current-release-v1767-'));
process.env.OWNER_CONTROL_CENTER_TOKEN='owner-current-release-v1767-token-123456789';
...
(async()=>{
 ...
 const v=truth.validate();assert.equal(v.ok,true,v.errors.join('\n'));assert.equal(v.releaseVersion,'1.7.83');assert.equal(v.sourceVersion,'1.7.82');assert.equal(v.launchState,'NO_GO');assert.equal(v.deploymentAuthorized,false);
 ...
 const cont=fs.readFileSync(path.join(root,'CONTINUATION_PROMPT_V1.7.76.md'),'utf8');assert.equal((cont.match(/^SMARTER JUSTICE$/gm)||[]).length,1);assert(cont.includes('EXPECTED NEXT VERSION IF BASE REMAINS CURRENT: v1.7.77'));assert(cont.includes('smarter-justice-v1.7.75.zip'));assert(cont.includes('SJP-2026-08-02-C15-P37-D11-V13'));assert(cont.trimEnd().endsWith('END OF SMARTER JUSTICE CENTRAL V12 CONTINUOUS-IMPROVEMENT FINAL CONTINUATION'));
 ...
 try{
  ...
  const allowed=await request(base,'/api/owner/current-release-truth',h);assert.equal(allowed.status,200);assert.equal(allowed.data.appVersion,'1.7.83');assert.equal(allowed.data.currentReleaseTruth.validation.ok,true);assert.equal(allowed.data.currentReleaseTruth.selectedBase.version,'1.7.82');
  const cc=await request(base,'/api/owner/control-center',h);assert.equal(cc.status,200);assert.equal(cc.data.version,'1.7.83');assert.equal(cc.data.currentReleaseTruth.validation.ok,true);assert.equal(cc.data.currentReleaseTruth.publicDisplay.currentVersion,'1.7.83');
 }finally{await new Promise(r=>server.close(r));}
 console.log('current-release-truth-v1767.test.js passed');
